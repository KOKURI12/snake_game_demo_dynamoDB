...
| v2.5 | Phase 6-B: Rebirth Fever Mode / Buff Bar UX / Combo Charge | リリース済み |
| v2.5.1 | Rebirth Charge / Guarantee System(Lv6+ で Normal Core 10 個取得 → 確定 Rebirth) | リリース済み |
| v2.5.2 | Level Threshold Curve(Lv6 以降の必要 score を段階的に増加) | リリース済み |
| v2.5.3 | Level-based Slow Assist + Soft Anti-Stall Timer | リリース済み |
| **v2.5.4** | **Rebirth Charge Electric Body UX(snake body 上の amber/gold glow + electric arc 表現)** | **実装・PC/スマホ確認済み** |
...
v2.5 公開後の実機プレイ確認を経て、Rebirth 体験の完成度を段階的に磨き上げました。
いずれもフロントエンド(`frontend/`)内のみの変更で、backend / infrastructure / API / DynamoDB / Redis / GitHub Actions / Ranking 仕様には影響していません。

#### v2.5.1: Rebirth Charge / Guarantee System

- Lv6 以降、Normal Core を 10 個取得すると `REBIRTH READY` 状態に到達
- READY 状態では次回の specialFood spawn 抽選で Rebirth Core が確定出現(spawn timer は維持)
- Rebirth 取得時のみ Charge をリセット。Slow Core / TTL 切れ / pause では維持
- 進捗は内部 integer step(0〜10)で管理し、小数誤差なく READY 判定が成立

#### v2.5.2: Level Threshold Curve

- Lv1〜Lv6 は従来通り 5 点ごとの上昇を維持
- Lv6 以降は段階的に必要 score を増加(Lv7=35 / Lv8=50 / Lv9=70 / Lv10=95)
- Rebirth Fever / Combo / Charge の戦略を高レベル帯でじっくり味わえる時間を確保
- speed テーブル / BGM BPM テーブル / level-flash 演出は変更なし

#### v2.5.3: Level-based Slow Assist + Soft Anti-Stall Timer

- Lv6+ の Rebirth/Slow 抽選比率を Lv 別に微調整し、高 Lv ほど Slow Core が出やすくなる救済を導入
  - Lv6-7: Rebirth base 0.55 / max 0.80
  - Lv8-9: Rebirth base 0.50 / max 0.75
  - Lv10: Rebirth base 0.45 / max 0.70
- 最後の Normal Core 取得からの経過時間に応じて specialFood spawn timer の進行倍率を変える Soft Anti-Stall Timer を導入(0-5秒: 100% / 5-10秒: 25% / 10秒以上: 停止)
- timer は reset せず進行速度のみ調整、待機戦略を緩やかに抑制
- specialFood TTL / spawn probability / spawn delay 等の数値は無変更

#### v2.5.4: Rebirth Charge Electric Body UX

- Rebirth Charge の進捗を外付け progress bar ではなく、**snake 本体上の amber/gold glow + electric arc** として表現
- charged segment(tail 側から head 側へ伸びる)に外側 halo / 縁 rim / 控えめな overlay を重ね、緑のベース色を残したまま「光をまとう」体感に
- 隣接 charged segment 間に短い jagged な lightning line(太い gold glow + 細い white-gold 芯線の 2 層)を時間に応じて点滅描画し、「電流が tail → head に流れる」アーケード演出を実現
- 通常進捗の `CHARGE 50%〜90%` chip 表示は廃止。Buff Bar には READY 時のみ `REBIRTH READY` chip を表示
- READY 時は全身 halo + 強化 head aura(gold radial gradient + pulse)+ electric arc 高密度
- `prefers-reduced-motion` 環境では electric arc / wave / aura pulse を停止し、静止 halo / overlay / rim / aura のみで charge 状態と READY 状態を表現
- ゲームロジック(`rebirthChargeSteps` 加算・reset・READY 判定・確定 spawn・Slow Assist・Soft Anti-Stall Timer・Level Threshold)はすべて v2.5.1 以降の仕様を維持

#### v2.5.1〜v2.5.4 で変更したファイル

- `frontend/index-v2.html`(Buff Bar の chip 構成を v2.5.4 で再整理)
- `frontend/css/style-v2.css`(`#buff-rebirth-ready` の amber/gold スタイル等)
- `frontend/js/snake-v2.js`(各バージョンのロジック・描画演出を順次追加・調整)

#### v2.5.1〜v2.5.4 で変更しなかったもの

- `frontend/js/audio-v2.js` / `frontend/js/api.js`
- `backend/` / `infrastructure/`
- Ranking API / Score 登録 API / DynamoDB / ElastiCache Redis
- GitHub Actions
- v2.1 rollback files / v2.3 rollback snapshot files
...
│   ├── index-v2.html                # v2.2〜v2.5.4 メインページ(現 Default Root Object)
│   ├── index-v2.3.html              # v2.3 rollback snapshot
│   ├── css/
│   │   ├── style.css                # v2.1 スタイル(rollback 用に保持)
│   │   ├── style-v2.css             # v2.2〜v2.5.4 スタイル
│   │   └── style-v2.3.css           # v2.3 rollback snapshot
│   └── js/
│       ├── api.js                   # API Gateway クライアント(共通)
│       ├── snake.js                 # v2.1 ゲームロジック(rollback 用に保持)
│       ├── snake-v2.js              # v2.2〜v2.5.4 ゲームロジック (Special Food / Fever / Buff Bar / Combo Charge / Rebirth Charge Electric Body UX / Dev Mode)
...
### Special Food System(v2.5 / v2.5.4)
...
#### Rebirth Charge / REBIRTH READY(v2.5.1〜)

Lv6 以降、Normal Core を取得するごとに内部的な Rebirth Charge が蓄積し、10 個取得すると `REBIRTH READY` 状態に到達します。
READY 状態では、次に出現する特殊 Core が必ず Rebirth Core になり、ハイリスク・ハイリターンのループを能動的に組み立てられます。
Charge は Rebirth Core を実際に取得した時のみリセットされ、Slow Core 取得や時間切れではリセットされません。

#### Rebirth Charge Electric Body UX(v2.5.4)

Rebirth Charge の進捗は画面上の外付けバーではなく、**snake 本体の見た目変化**として表現されます。

- charged 部分(tail 側から head 側に伸びる)に外側 halo と縁の rim が gold で発光
- 隣接 segment 間に短い lightning(electric arc)が tail 側から head 側へ流れるように瞬く
- snake 本体の緑色はベースとして残り、「色が変わる」ではなく「電流をまとう」体感
- `REBIRTH READY` 到達時は全身が halo + rim で発光し、head 周囲に gold aura が pulse、Buff Bar に `REBIRTH READY` chip を表示
- `prefers-reduced-motion` 環境では電流アニメーションを停止し、静止 glow / aura のみで charge 状態を伝える

なお Rebirth Charge の加算 / リセット / READY 判定 / READY 中の Rebirth 確定 spawn 等のゲームロジックは v2.5.1 の仕様を維持しており、backend / API / Ranking には影響しません。
